...
exports.run = (client, message, args) => {
  ...
  if (!modlog) return message.reply('`kayitlog` kanalını bulamıyorum.');
  if (!userr) return  message.channel.sendEmbed(new Discord.RichEmbed().setDescription(`Lütfen bir kullanıcı belirt .\n**Örnek: ` + ayarlar.prefix + `kız @kullanıcı**`).setColor("RANDOM").setAuthor(`${message.author.username} tarafından istendi.`, message.author.avatarURL).setTimestamp());
  message.delete();
	var role = message.guild.roles.find(role => role.id === "643091549110140939");
  ...
  userr.addRole(role).then((member) => {
   ...
   const ke = new Discord.RichEmbed()
     ...
     .addField('Şu Olarak Kayıt Edildi', '<@&643091549110140939>');
